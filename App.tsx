...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] pb-24">
      ...
      {/* SOS Header */}
      <header className="bg-[#E63946] text-white pt-10 pb-10 px-8 shadow-2xl rounded-b-[50px] mb-10 relative overflow-hidden">
        ...
        <div className="flex justify-between items-center mb-6 relative z-10">
          <div className="text-yellow-400 font-black text-[12px] tracking-[0.2em] uppercase drop-shadow-sm">Prevenção de Perdas</div>
          <div className="text-white text-xs font-bold opacity-70 tracking-widest">{new Date().toLocaleDateString('pt-BR')}</div>
        </div>
        ...
        <div className="flex justify-between items-center relative z-10">
          ...
          <div className="relative group ml-4">
            ...
            <img 
              ...
              onError={(e) => {
                // Fallback image source if the specific one is not found locally
      ...
      <main className="px-5">
        {successData && activeTab === Tab.HISTORY && (
          <div className="bg-green-600 text-white p-8 rounded-[40px] mb-10 shadow-2xl animate-in fade-in zoom-in duration-700">
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-2xl font-black leading-none">Protocolo<br/>CODD-{successData.id}</h2>
              <span className="text-3xl">✅</span>
            </div>
            <p className="text-xs font-bold opacity-80 mb-6 leading-relaxed uppercase">Ocorrência registrada e tagueada automaticamente para a equipe de resposta.</p>
            <button 
              onClick={copyToWhatsApp}
              className="w-full py-4 bg-white text-green-700 font-black rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition shadow-lg text-xs"
            >
      ...
      <nav className="fixed bottom-6 left-6 right-6 bg-slate-900 text-white px-8 py-4 flex justify-between items-center z-50 rounded-[35px] shadow-2xl">
        <button 
          onClick={() => setActiveTab(Tab.NEW_CLAIM)}
          className={`flex flex-col items-center gap-1 transition-all px-6 py-2 rounded-2xl ${
            activeTab === Tab.NEW_CLAIM 
            ? 'bg-[#f8f9fa] text-red-600 scale-105 shadow-[0_0_15px_rgba(255,255,255,0.1)]' 
            : 'text-slate-500 opacity-50'
          }`}
        >
          <span className="text-xl">🚨</span>
          <span className={`text-[8px] font-black uppercase tracking-tighter ${activeTab === Tab.NEW_CLAIM ? 'text-red-700' : ''}`}>Aviso</span>
        </button>
        ...
        >
          <span className="text-xl">📊</span>
          <span className={`text-[8px] font-black uppercase tracking-tighter ${activeTab === Tab.HISTORY ? 'text-red-700' : ''}`}>Histórico</span>
        </button>

        <button 
          onClick={() => (window as any).zE?.('messenger', 'open')}
          className="flex flex-col items-center gap-1 text-slate-500 opacity-50 active:scale-110 px-6 py-2"
        >
          <span className="text-xl">💬</span>
